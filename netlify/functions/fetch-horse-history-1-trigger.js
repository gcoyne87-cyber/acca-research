...
// fetch-horse-history-trigger-background and return -- the same shim pattern
// as form-summary-trigger.js, and for the same reason: pairing "schedule"
...
// checks the build secret and calls fetch-horse-history-1-background.js)
// for the real run.
...
function triggerRun() {
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      path: '/.netlify/functions/fetch-horse-history-trigger-background',
      method: 'POST',
      headers: {
...
exports.handler = async function() {
  try {
    ...
    console.log('[fetch-horse-history-1-trigger] POST to fetch-horse-history-trigger-background ->', result.statusCode);
    return { statusCode: 200, body: JSON.stringify({ triggered: true, upstreamStatus: result.statusCode }) };
  } catch (e) {
    console.error('[fetch-horse-history-1-trigger] failed to trigger fetch-horse-history-trigger-background:', e.message);
