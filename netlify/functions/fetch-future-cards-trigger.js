...
// fetch-future-cards-trigger-background and return -- the same shim pattern
...
function triggerRun() {
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      path: '/.netlify/functions/fetch-future-cards-trigger-background',
      method: 'POST',
      headers: {
...
exports.handler = async function() {
  try {
    ...
    console.log('[fetch-future-cards-trigger] POST to fetch-future-cards-trigger-background ->', result.statusCode);
    return { statusCode: 200, body: JSON.stringify({ triggered: true, upstreamStatus: result.statusCode }) };
  } catch (e) {
    console.error('[fetch-future-cards-trigger] failed to trigger fetch-future-cards-trigger-background:', e.message);
